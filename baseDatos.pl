...
incioConversacionAux(Lista,[]):-incioConversacion(Lista,[]),deporteSolicitado(Deporte),nivelarCliente(Nivel),padecimientoSolicitado(Padecimiento),rutina(Nombre,Deporte,Padecimiento,Nivel,Dias, Detalle),write('Nombre de la rutina: '),write(Nombre),nl,write('Dias a la semana de entrenamiento: '),write(Dias),nl,write(Detalle). 
...
seleccion(Lista,Vacio,Deporte):-negacion(Lista,Vacio),write('Puede escoger entre correr,ciclismo,natacion'),opcionesDeporte(Deporte).
...
padecimientos(Lista,[],Padecimiento):- afirmacion(Lista,Resto),padecimiento(Resto,Vacio),atomics_to_string(Resto,Padecimiento).
padecimientos(Lista,[],Padecimiento):- afirmacion(Lista,Vacio),padecimientosAux(Padecimiento).
padecimientos(Lista,[],Padecimiento):- negacion(Lista,Vacio),atomics_to_string(Vacio,Padecimiento).
...
nivelarCliente(Nivel):- write('Cuantas veces a la semana hace ejercicio'),nl ,read_string(user_input, '\n', '.', _, Cantidad), buscarNivel(Nivel,Cantidad).
