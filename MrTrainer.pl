...
incioConversacionAux(Lista,[]):-incioConversacion(Lista,[]),deporteSolicitado(Deporte),nivelarCliente(Nivel),padecimientoSolicitado(Padecimiento),buscarRutina(Nombre,Deporte,Padecimiento,Nivel,Dias, Detalle).
...
seleccion(Lista,Vacio,Deporte):-negacion(Lista,Vacio),write('Adios'),nl,break.
seleccion(Lista,Vacio,Deporte):-afirmacion(Lista,Resto),deporte(Resto,Vacio),atomics_to_string(Resto,Deporte).  

opcionesDeporte(LowerCase):- nl, read_string(user_input, '\n', '.', _, Str),string_lower(Str,LowerCase),atomic_list_concat(Deporte, ' ', LowerCase),deporte(Deporte,Vacio).
...
padecimientoSolicitado(Padecimiento):- write("No he reconocido su respuesta, por favor prube con otra como por ejemplo: escoliosis/cardiopata/hipertension"),nl,padecimientoSolicitado(Padecimiento).
...
rutina('Preparación para 120k o más',"ciclismo", "escoliosis", 'intermedio intenso', 4,
       ...
       Domingo: - Descanso
       '). 
rutina('Mantener desarrollo para 120km o más',"ciclismo", "escoliosis", 'avanzado', 4,
       'Lunes: - Descanso\n
...
buscarRutina(Nombre,Deporte,Padecimiento,Nivel,Dias, Detalle):- rutina(Nombre,Deporte,PadecimientoAux,Nivel,Dias, Detalle),Padecimiento\=PadecimientoAux,write('Le recomiendo la siguiente rutina: '),nl,write('Nombre de la rutina: '),write(Nombre),nl,write('Dias a la semana de entrenamiento: '),write(Dias),nl,write(Detalle).
buscarRutina(Nombre,Deporte,Padecimiento,Nivel,Dias, Detalle):- write("Lo siento de momento no tengo ninguna rutina que se adapte a sus necesides"),nl.
